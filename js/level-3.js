(function initLevel3() {
  ...
  const GOAL_TREE = TreeOptimal.LEVEL3_GOAL;
  const NUMBER_POOL = TreeOptimal.LEVEL3_POOL;
  const TOTAL_STEPS = NUMBER_POOL.length;
  const SLANT_LAYOUT = TreeMap.SLANT_LAYOUT;
  const SLANT_GAME_LAYOUT = TreeMap.SLANT_GAME_LAYOUT;
  const INTRO_LAYOUT = { ...SLANT_GAME_LAYOUT, padX: 64, padY: 56 };
  const COMPARE_LAYOUT = { ...SLANT_GAME_LAYOUT, padX: 64, padY: 56, camera: false };
  ...
  const GOAL_PAN_IDS = ["g4", "g3", "g2", "g1"];
  ...
  function ensureGoalTree() {
    ...
    if (!goalMap) {
      ...
      goalMap = TreeMap.mountGoalStructure(goalStage, data, SLANT_GAME_LAYOUT);
    }
    return goalMap;
  }
  ...
  function syncTreeDisplay(forValue) {
    ...
    const opts = { showAll: true, layout: SLANT_GAME_LAYOUT };
    let pending = null;
  ...
  async function runGoalIntro() {
    ...
    const tourLoop = (async () => {
      while (!dismissed) {
        ...
        await introMap?.panToFit({ duration: prefersReducedMotion ? 0 : 650, padding: 48 });
        if (dismissed) break;
        await delay(prefersReducedMotion ? 200 : 500);

        for (const id of GOAL_PAN_IDS) {
          if (dismissed) break;
          if (goalIntroHint) goalIntroHint.textContent = "Following the goal path…";
  ...
  function prepareChoices() {
    currentChoices = TreeOptimal.getChoicePair(remaining);
  ...
  function setupQuestionCards() {
    const { goalPick, altPick } = prepareChoices();

    if (questionTitle) {
      if (!playerTree) {
        questionTitle.innerHTML = "Which number starts your tree?";
      } else {
    ...
    const goalSeq = TreeOptimal.getInsertPath(playerTree, goalPick);
    const altSeq = TreeOptimal.getInsertPath(playerTree, altPick);
  ...
  function buildTileChart() {
    const chart = chosenValue != null ? TreeOptimal.getInsertPath(playerTree, chosenValue) : [];
  ...
  function resetCards() {
    ...
    cards.forEach((c) => {
      c.disabled = false;
      c.hidden = false;
      c.classList.remove("is-crossed", "is-vanishing", "is-auto-picked");
    });
  ...
  function handleL3Pick(card) {
    ...
    const feedbackMs = prefersReducedMotion ? 200 : 320;
  ...
  function onChoiceTimeout() {
    ...
    if (picked) {
      playL3PickFeedback(picked);
    }
    setPrompt(`Time's up — auto-picked ${chosenValue}`);
    const feedbackMs = prefersReducedMotion ? 200 : 320;
    setTimeout(() => {
      advanceAfterChoice(`Placing ${chosenValue}…`, feedbackMs);
    }, prefersReducedMotion ? 60 : 140);
  }
  ...
  function showQuestion() {
    ...
    setPrompt(`${CHOICE_HINT} · ${remaining.size} left in pool`);
    startChoiceTimer();
  ...
  function startSequence() {
    ...
    const promptAfter = song
      ? `Tap on the beat — place ${chosenValue} · ${song}`
