(function initLevel4() {
  ...
  const GOAL_TREE = TreeOptimal.LEVEL4_GOAL;
  const NUMBER_POOL = TreeOptimal.LEVEL4_POOL;
  const GOAL_ORDER = TreeOptimal.LEVEL4_GOAL_ORDER;
  const GOAL_PAN_IDS = TreeOptimal.collectGoalPanIds(GOAL_TREE);
  const TOTAL_STEPS = NUMBER_POOL.length;
  const GAME_LAYOUT = TreeMap.SLANT_GAME_LAYOUT_L4;
  const INTRO_LAYOUT = { ...GAME_LAYOUT, padX: 64, padY: 56 };
  const COMPARE_LAYOUT = { ...GAME_LAYOUT, padX: 64, padY: 56, camera: false };
  ...
  function setupQuestionCards() {
    ...
    const goalSeq = TreeOptimal.getInsertPath(playerTree, goalPick);
    const wrongSeq = formatSeq([...TreeOptimal.getInsertPath(playerTree, wrongPick)].reverse());
  ...
  function onChoiceTimeout() {
    ...
    if (currentChoices?.mode === "l2") {
      ...
      setTimeout(() => {
        autoSelectCorrect();
        advanceAfterChoice(`Placing ${chosenValue}…`, 1100);
      }, 450);
    } else {
